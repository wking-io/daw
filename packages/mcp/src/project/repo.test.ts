...
const makeTestProject = (id: string, name: string): Project.ProjectWithTimestamps => ({
...
const makeProjectSummary = (id: string, name: string): Project.ProjectSummary => ({
  id: Ids.ProjectId.make(id),
  name,
  version: Versions.ProjectVersion.make(1),
  createdAt: DateTime.unsafeNow(),
...
const makeCreateCommand = (projectId: Ids.ProjectId, name: string): ProjectCreateCommand => ({
...
const makeTestLayer = (overrides: {
  ...
  const defaultGet: GetFn = () => Effect.succeed(makeTestProject("test-id", "Test Project"));
  const defaultCreate: CreateFn = () => Effect.succeed(makeTestProject("test-id", "Test Project"));
  const defaultRemove: RemoveFn = () => Effect.succeed(makeTestProject("test-id", "Test Project"));
...
describe("ProjectRepository", () => {
  ...
  describe("create", () => {
    ...
    it("returns BadRequest error", async () => {
      ...
      const result = await Effect.gen(function* () {
        const repo = yield* ProjectRepository;
        return yield* repo.create(makeCreateCommand(Ids.ProjectId.make("test"), "Test"));
    ...
    it("returns NotAcceptable error", async () => {
      ...
      expect((result as ApiError.NotAcceptable).detail).toBe("Server rejected response");
    });
  });
});
