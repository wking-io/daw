  ...
  "mcp/ProjectRepository",
  {
    effect: Effect.gen(function* () {
      ...
      const get = (projectId: Ids.ProjectId) =>
        api.project.get({ path: { projectId } }).pipe(Effect.mapError(mapError));

      const create = (payload: ProjectCreateCommand) =>
        api.project.create({ payload }).pipe(Effect.mapError(mapError));

      const remove = (projectId: Ids.ProjectId, payload: ProjectDeleteCommand) =>
        api.project.delete({ path: { projectId }, payload }).pipe(Effect.mapError(mapError));
