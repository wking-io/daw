...
export const map = <A extends number, B extends number>(r: Range<A>, f: (t: A) => B): Range<B> => ({
  start: f(r.start),
  end: f(r.end),
});

export const transformStart = <A extends number>(r: Range<A>, f: (t: A) => A): Range<A> => ({
  ...r,
  start: f(r.start),
});

export const transformEnd = <A extends number>(r: Range<A>, f: (t: A) => A): Range<A> => ({
  ...r,
  end: f(r.end),
});

export const move = <A extends number>(N: Numeric<A>, r: Range<A>, delta: A): Range<A> => ({
