...
export type ProjectWithTimestamps = Schema.Schema.Type<typeof ProjectWithTimestamps>;
...
function removeById<T extends { id: string }>(arr: readonly T[], id: string): T[] {
  return arr.filter((item) => item.id !== id);
}

function updateNestedById<Parent extends { id: string }, Child extends { id: string }>(
  arr: readonly Parent[],
  parentId: string,
  ...
  return arr.map((parent) => {
    ...
    return {
      ...parent,
      [childArrayKey]: children.map((child) => (child.id === childId ? updater(child) : child)),
    };
  });
}
...
export function evolve(project: Project, event: EditorEvent): Project {
  switch (event.t) {
    ...
    case "project.tracksReordered": {
      const trackMap = new Map<string, TrackType>(project.tracks.map((t) => [t.id, t] as const));
    ...
    case "midi.patternRenamed":
      return {
        ...project,
        midiPatterns: updateById(project.midiPatterns, event.patternId, (p) => ({
          ...p,
          name: event.name,
        })),
      };
    ...
    case "automation.pointMoved":
      return {
        ...project,
        automationLanes: updateNestedById<AutomationLaneType, AutomationPointType>(
          project.automationLanes,
          event.laneId,
          "points",
          event.pointId,
...
export function create(command: ProjectCreate): ProjectCreated {
  return {
    ...
    project: {
      ...
      bpm: command.bpm ?? 120,
      timeSignature: command.timeSignature ?? TimeSignature.make({ numerator: 4, denominator: 4 }),
      tracks: [],
      clips: [],
      midiPatterns: [],
      automationLanes: [],
...
export function decide(project: Project, command: EditorCommandPayload): readonly EditorEvent[] {
  switch (command.t) {
    case "project.delete":
      return [
        {
    ...
    case "clip.resize":
      return [{ t: "clip.resized", clipId: command.clipId, span: command.span }];
    ...
    case "audioFile.unregister":
      return [{ t: "audioFile.unregistered", audioFileId: command.audioFileId }];
