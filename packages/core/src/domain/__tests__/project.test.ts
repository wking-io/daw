...
describe("Project schema", () => {
  ...
  it("rejects bpm below 20", () => {
    expect(() => Schema.decodeUnknownSync(Project)({ ...validProjectJson, bpm: 19 })).toThrow(
      "between(20, 999)",
    );
  });
...
describe("Project.evolve", () => {
  describe("project events", () => {
    ...
    it("handles project.tracksReordered", () => {
      ...
      const event: EditorEvent = {
        ...
        trackIds: [TrackId.make("track-3"), TrackId.make("track-1"), TrackId.make("track-2")],
      };
  ...
  describe("track events", () => {
    ...
    it("handles track.clipsReordered", () => {
      ...
      const event: EditorEvent = {
        ...
        trackId: TrackId.make("track-1"),
        clipIds: [ClipId.make("clip-3"), ClipId.make("clip-1"), ClipId.make("clip-2")],
      };
  ...
  describe("midi events", () => {
    ...
    it("handles midi.noteDeleted", () => {
      const project = {
        ...createBaseProject(),
        midiPatterns: [createMidiPattern("pattern-1", [createMidiNote("note-1")])],
      };
      const event: EditorEvent = {
        t: "midi.noteDeleted",
        patternId: PatternId.make("pattern-1"),
  ...
  describe("automation events", () => {
    it("handles automation.laneCreated", () => {
      ...
      expect(first(result.automationLanes).id).toBe(AutomationLaneId.make("lane-1"));
    });
    ...
    it("handles automation.pointDeleted", () => {
      const project = {
        ...createBaseProject(),
        automationLanes: [
          createAutomationLane("lane-1", "track-1", [createAutomationPoint("point-1")]),
        ],
      };
      const event: EditorEvent = {
  ...
  describe("track commands", () => {
    ...
    it("returns track.clipsReordered event", () => {
      const project = {
        ...createBaseProject(),
        tracks: [createTrack("track-1")],
        clips: [createClip("clip-1", "track-1"), createClip("clip-2", "track-1")],
      };
      const command: EditorCommandPayload = {
