...
export function Select(
  ...
  return (props: { value: string; class?: string }) => {
    ...
    return (
      <div
        id={id}
        class={cn("flex focus-within:ring-2 focus-within:ring-blue-500/50 rounded-md", props.class)}
      >
        <button
          tabindex={-1}
          type="button"
          disabled={disabled}
          on={{
            click: () => handleChange(prevOption(setup.options, props.value), initialValue),
          }}
          class={cn(
            ...
            disabled ? "cursor-not-allowed opacity-50" : "hover:bg-layer-2 active:bg-layer-2",
          )}
        >
        ...
        <input
          ...
          on={{
            keydown: (e: KeyboardEvent) => {
              if (e.key === "ArrowLeft") {
                handleChange(prevOption(setup.options, props.value), initialValue);
              } else if (e.key === "ArrowRight") {
                handleChange(nextOption(setup.options, props.value), initialValue);
              }
            },
            focus: () => handleFocus(),
        ...
        <button
          ...
          disabled={disabled}
          on={{
            click: () => handleChange(nextOption(setup.options, props.value), initialValue),
          }}
          class={cn(
