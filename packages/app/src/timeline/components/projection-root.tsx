...
export function ProjectionRoot(handle: Handle<ProjectionRootContext>) {
  ...
  function getScale() {
    ...
    return Projection.scaleFor(Px.Numeric, rootCtx.timeline.view.size, Px.Px(size.width));
  }
  ...
  function onScroll() {
    ...
    const nextStart = Scroll.fromScroll(Px.Numeric, Px.Px(containerEl.scrollLeft), scale);
  ...
  return (props: { children?: RemixNode; height?: number; class?: string }) => {
    ...
    const scale = getScale();
    const contentWidth = Scroll.width(Px.Numeric, rootCtx.timeline.size, scale);

    // State → DOM: sync scroll position after render
    handle.queueTask(() => {
      if (!containerEl || rootCtx.isInteracting) return;

      const nextScrollLeft = Scroll.toScroll(Px.Numeric, rootCtx.timeline.view.start, scale);
