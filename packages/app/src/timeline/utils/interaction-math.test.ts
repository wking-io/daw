...
describe("timeline/utils/interaction-math", () => {
  ...
  describe("zoomFactorFromDelta", () => {
    it("returns 1 for zero delta (no zoom)", () => {
      expect(zoomFactorFromDelta(0, Px.Px(2))).toBe(1);
    });

    it("returns factor > 1 for negative delta (zoom in)", () => {
      const factor = zoomFactorFromDelta(-100, Px.Px(16));
    ...
    it("doubles at delta = -logRate when viewSize = 1 (log2(2) = 1)", () => {
      ...
      expect(zoomFactorFromDelta(-50, Px.Px(1))).toBeCloseTo(2);
    });

    it("halves at delta = +logRate when viewSize = 1 (log2(2) = 1)", () => {
      expect(zoomFactorFromDelta(50, Px.Px(1))).toBeCloseTo(0.5);
    });

    it("is symmetric: inverse deltas produce reciprocal factors", () => {
      const zoomIn = zoomFactorFromDelta(-200, Px.Px(16));
      const zoomOut = zoomFactorFromDelta(200, Px.Px(16));
      expect(zoomIn * zoomOut).toBeCloseTo(1);
    });

    it("zooms slower when view is large (zoomed out)", () => {
      const factorSmallView = zoomFactorFromDelta(-100, Px.Px(4)); // log2(4)=2
      const factorLargeView = zoomFactorFromDelta(-100, Px.Px(256)); // log2(256)=8
    ...
    it("zooms faster when view is small (zoomed in)", () => {
      const factorSmallView = zoomFactorFromDelta(-100, Px.Px(1)); // log2(2)=1
      const factorMediumView = zoomFactorFromDelta(-100, Px.Px(16)); // log2(17)≈4.09
    ...
    it("keeps scaling below quarter note (no clamp)", () => {
      ...
      const factorAt1 = zoomFactorFromDelta(-100, Px.Px(1));
      const factorAt05 = zoomFactorFromDelta(-100, Px.Px(0.5));
