...
import { point, rect, stroke, textStyle, pointInRect, nodeBounds } from "./types";
...
describe("timeline/scene/types", () => {
  ...
  describe("textStyle", () => {
    ...
    it("handles all align values", () => {
      const aligns: CanvasTextAlign[] = ["start", "end", "left", "right", "center"];
    ...
    it("handles all baseline values", () => {
      ...
      for (const baseline of baselines) {
        expect(textStyle("12px Arial", "black", undefined, baseline).baseline).toBe(baseline);
      }
    });
  });
