...
export class ProjectEventBus extends Effect.Service<ProjectEventBus>()("server/ProjectEventBus", {
  effect: Effect.gen(function* () {
    ...
    const subscribe = (projectId: Ids.ProjectId) =>
      Stream.fromPubSub(pubsub).pipe(Stream.filter((msg) => msg.projectId === projectId));
