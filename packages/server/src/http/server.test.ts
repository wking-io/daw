...
const makeLayer = () => {
  ...
  const apiWithDeps = ApiLive.pipe(Layer.provide(projectLayers), Layer.provide(configLayer));
...
describe("HttpEndpoints", () => {
  ...
  it("subscribe endpoint returns server.subscribed event", async () => {
    ...
    const res = await handler(
      new Request(`http://localhost/api/events/subscribe?fromVersion=0&projectId=${projectId}`, {
        headers: { Authorization: `Bearer ${TEST_TOKEN}` },
      }),
    );
